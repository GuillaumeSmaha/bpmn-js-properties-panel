...
function getPropertyValue(element, property) {
  ...
  // scope
  var scopeElements = bo;
  if (bindingScope) {
    var scope = findExtension(bo, bindingScope);
    if (!scope) {
      // set scope as main element
      scopeElements = scope;
    }
  }
  ...
  if (bindingType === CAMUNDA_PROPERTY_TYPE) {
    camundaProperties = findExtension(scopeElements, 'camunda:Properties');
  ...
  if (IO_BINDING_TYPES.indexOf(bindingType) !== -1) {
    inputOutput = findExtension(scopeElements, 'camunda:InputOutput');
  ...
  if (IN_OUT_BINDING_TYPES.indexOf(bindingType) != -1) {
    ioElement = findCamundaInOut(scopeElements, binding);
  ...
  if (bindingType === CAMUNDA_EXECUTION_LISTENER_TYPE) {
    var executionListener = findExtension(scopeElements, 'camunda:ExecutionListener');
...
function setPropertyValue(element, property, value, bpmnFactory) {
  ...
  // scope
  var scopeElements = extensionElements;
  if (bindingScope) {
    var scope = findExtension(extensionElements, bindingScope);

    // create inputOutput element, if it do not exist (yet)
    if (!scope) {
      // create scope element, if it do not exist (yet)
      scope = bpmnFactory.create(bindingScope);

      updates.push(cmdHelper.addElementsTolist(
        element, extensionElements, 'values', scope
      ));
  ...
  if (bindingType === CAMUNDA_PROPERTY_TYPE) {
    ...
    if (!camundaProperties) {
      ...
      updates.push(cmdHelper.addElementsTolist(
        element, scopeElements, 'values', [ camundaProperties ]
      ));
    }
  ...
  if (IO_BINDING_TYPES.indexOf(bindingType) !== -1) {
    ...
    if (!inputOutput) {
      ...
      updates.push(cmdHelper.addElementsTolist(
        element, scopeElements, 'values', inputOutput
      ));
    }
  }
