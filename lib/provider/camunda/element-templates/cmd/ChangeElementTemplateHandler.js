...
function ChangeElementTemplateHandler(modeling, commandStack, bpmnFactory) {
  ...
  function updateScopeElements(element, scope) {
    ...
    updateIoMappings(scopeElement, scopeElement, scope);

    // update camunda:properties
    updateCamundaProperties(scopeElement, scopeElement, scope);

    // update other properties (bpmn:condition, camunda:async, ...)
    updateProperties(scopeElement, scopeElement, scope);

    // update camunda:in and camunda:out
    updateInOut(scopeElement, scopeElement, scope);

    // update camunda:executionListener
    updateExecutionListener(scopeElement, scopeElement, scope);

    var oldScope = findExtension(element, scopeName);
  ...
  this.preExecute = function(context) {
    ...
    if (newTemplate) {
      ...
      if (newTemplate.scopes) {
        newTemplate.scopes.forEach(function(s) {

          updateScopeElements(element, extensionElements, s);
