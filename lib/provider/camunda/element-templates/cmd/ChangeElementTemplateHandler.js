...
var createCamundaProperty = require('../CreateHelper').createCamundaProperty,
    ...
    createCamundaTag = require('../CreateHelper').createCamundaTag;

var findCamundaTag = require('../Helper').findCamundaTag;
...
function ChangeElementTemplateHandler(modeling, commandStack, bpmnFactory) {
  ...
  function updateTag(element, newTemplate) {

    newTemplate.properties.forEach(function(p) {
      var binding = p.binding,
          bindingType  = binding.type;

      if (bindingType === 'camunda:tag') {
        var extensionElements = getOrCreateExtensionElements(element);

        var newTag = createCamundaTag(
            binding, p.value, bpmnFactory
          );
        var oldTag = findCamundaTag(extensionElements, binding);
